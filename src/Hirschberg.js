...
function merge(alignment1, alignment2) {
    ...
    var b = alignment1.substring(length * 2 + 2, alignment1.length) + alignment2.substring(length * 2 + 2, alignment2.length);
    var middle = '';
...
function hirschberg(short, long, delta, indent_i, indent_j, level) {
    ...
    d[level].push([indent_i, indent_j, short.length + indent_i, long.length + indent_j, short, long]);
    console.log('d[level]: ', d[level]);
    if (long.length > 1) {
        const psR = prefixSuffix(short, long, delta);
        var i = psR[0];
        var j = psR[1];
        var a = hirschberg(short.substring(0, i), long.substring(0, j), delta, indent_i, indent_j, level + 1);
        var b = hirschberg(short.substring(i, short.length), long.substring(j, long.length), delta, indent_i + i, indent_j + j, level + 1);
        return merge(a, b)
    } else {
        return globalAlignment(short, long, delta)[2]
    }
}
